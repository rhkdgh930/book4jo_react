...
const MyPage = () => {
  ...
  const handlePasswordUpdate = async () => {
    try {
      ...
      const response = await axios.put('http://localhost:8080/api/mypage/change-password',
        null,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
    ...
    <form>
      ...
      <div className="mypage-container">
        ...
        <div className="inputs-row">
          ...
          <button onClick={(e) => { e.preventDefault(); passwordEditMode ? handlePasswordUpdate() : togglePasswordEditMode(); }}>
        ...
        <div className="inputs-row">
          ...
          <button onClick={(e) => { e.preventDefault(); editMode.name ? handleUpdate('name') : toggleEditMode('name'); }}>
            {editMode.name ? '저장' : '변경'}
          </button>
        </div>
        <div className="inputs-row">
          <div className="info-item">
            ...
            {editMode.address ? (
              <input className="mypage-input" type="text" name="address" value={userInfo.address} onChange={handleChange} />
            ) : (
              <span className="span">{userInfo.address}</span>
            )}
          </div>
          <button onClick={(e) => { e.preventDefault(); editMode.address ? handleUpdate('address') : toggleEditMode('address'); }}>
        ...
        <div className="inputs-row">
          <div className="info-item">
            ...
            {editMode.phoneNumber ? (
              <input className="mypage-input" type="text" name="phoneNumber" value={userInfo.phoneNumber} onChange={handleChange} />
            ) : (
              <span className="span">{userInfo.phoneNumber}</span>
            )}
          </div>
          <button onClick={(e) => { e.preventDefault(); editMode.phoneNumber ? handleUpdate('phoneNumber') : toggleEditMode('phoneNumber'); }}>
