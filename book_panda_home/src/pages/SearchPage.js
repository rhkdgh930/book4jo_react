...
function SearchPage() {
    ...
    useEffect(() => {
        axios
            .get("/api/bookSales/search?keyword=" + searchParams.get("keyword") + "&page=" + (page - 1) + "&size=10")
            .then((res) => {
                    ...
                    <Link key={i} to={`/bookSalesDetail?id=${bookSales.id}`} style={{ color: "black" }}>
                        <div className={styles.bookContainer}>
                            ...
                            <div className={styles.infoContainer}>
                                <h6 className={styles.bookTitle}>{bookSales.bookInfo.title} {bookSales.bookInfo.subtitle}</h6>
