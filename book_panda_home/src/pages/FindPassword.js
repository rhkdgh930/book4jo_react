...
const FindPassword = () => {
  ...
  const sendEmail = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post("/api/api/users/sign-up/send-email", {
        userEmail: email,
      });
  ...
  const verifyCode = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post("/api/api/users/sign-up/verify-code", {
        userEmail: email,
        authCode: code,
      });
      setMessage(response.data);
      ...
      <Form>
        <div>
          ...
          <div className="email-input-wrapper" style={{ display: "flex", alignItems: "center" }}>
            ...
            <ButtonB type="button" onClick={sendEmail}>
              인증번호 받기
            </ButtonB>
          </div>
          {codeSent && (
            <div>
              ...
              <div className="code-input-wrapper" style={{ display: "flex", alignItems: "center" }}>
                ...
                <ButtonB type="button" onClick={verifyCode}>
                  확인
                </ButtonB>
              </div>
            </div>
          ...
          {codeVerified && (
            <div>
              ...
              <Button className="styled-button" type="button" onClick={changePassword}>
                비밀번호 변경
              </Button>
            </div>
          )}
